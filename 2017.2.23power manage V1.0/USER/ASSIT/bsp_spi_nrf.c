 ...
 void SPI_NRF_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC,ENABLE);
	...
	SPI2_Config(DMA_DISABLE,INTER_DISABLE);  //SPI2初始化
  ...
  GPIO_Init(GPIOC, &GPIO_InitStructure);     //临时定义CE 引脚为PC4
...
//	GPIO_InitStructure.GPIO_OType = GPIO_OType_OD;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(GPIOC, &GPIO_InitStructure); //临时定义引脚PC5
	//配置外部中断
...
void NRF_Tx_Dat()
{
	...
	NRF_CE_LOW();   	//NRF进入掉电模式，数据写入
	
	SPI_NRF_WriteReg(NRF_WRITE_REG+CONFIG,0x0e);  //切换为发送模式
						
  SPI_NRF_WriteBuf(WR_TX_PLOAD,tx_buff,TX_PLOAD_WIDTH);  /*写数据到TX BUF 最大 32个字节*/
  		
	NRF_CE_HIGH();//  NRF进入工作模式
} 
...
void EXTI9_5_IRQHandler()
{
	...
	if(EXTI_GetITStatus(EXTI_Line5) != RESET)
	{
		...
		{
			SPI_NRF_ReadBuf(RD_RX_PLOAD,rx_buff,RX_PLOAD_WIDTH);//读取数据
			SPI_NRF_WriteReg(FLUSH_RX,NOP);          //清除RX FIFO寄存器
		}
		else if((state&TX_DS) | (state&MAX_RT))    //发送成功或者达到最大发送次数
		{
			SPI_NRF_WriteReg(FLUSH_TX,NOP);    //清除TX FIFO寄存器 
			SPI_NRF_WriteReg(NRF_WRITE_REG+CONFIG,0x0f);  //进入接收模式	
		}
		else   //错误
		{
			SPI_NRF_WriteReg(FLUSH_RX,NOP);          //清除RX FIFO寄存器
